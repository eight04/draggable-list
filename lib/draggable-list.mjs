...
export function posToIndex(rects, startIndex, y, bound) {
...
export function applyTransform(list, startIndex, oldIndex, newIndex, len) {
  if (newIndex > oldIndex) {
    transform(false, oldIndex, Math.min(startIndex - 1, newIndex - 1));
    if (startIndex < list.length - 1) {
      transform(true, Math.max(oldIndex + 1, startIndex + 1), newIndex, `translateY(${-len}px)`);
    }
  } else {
    transform(false, Math.max(startIndex + 1, newIndex + 1), oldIndex);
    if (startIndex > 0) {
      transform(true, newIndex, Math.min(oldIndex - 1, startIndex - 1), `translateY(${len}px)`);
    }
  }
...
export function DraggableList(el, {bound, scrollContainer} = {}) {
  ...
  el.addEventListener('dragstart', e => {
    ...
    rects = [...el.children].map(el => {
      ...
      return {top: r.top + window.scrollY + scrollTop, bottom: r.bottom + window.scrollY + scrollTop};
    });
    itemSize = startIndex + 1 < rects.length ? rects[startIndex + 1].top - rects[startIndex].top:
      startIndex > 0 ? rects[startIndex].bottom - rects[startIndex - 1].bottom:
      0;
